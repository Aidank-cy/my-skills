...
description: Evaluate and improve the usability of existing front-end interfaces, either local source code or live websites by URL. Use when the user has front-end code or a URL and wants to understand why users struggle, find usability problems, improve UX, review UI, audit accessibility, evaluate design, audit a website, or make an interface easier to use. Also trigger on symptoms like abandoned forms, confusing pages, checkout complaints, conversion drops after redesigns, or "something feels off." Not for building new interfaces from scratch, performance optimization, or backend logic.
---
...
**For live websites (URL)** — Use the agent's available web or browser tools to retrieve and inspect the page. If the user provides a single URL, fetch that page. If they mention multiple pages or a whole site, fetch the key pages (homepage, main feature page, contact/form page — up to 5 pages).
...
- **Full evaluation with discussion** — default workflow when the user asks for an audit, UI review, UX review, or usability improvements.
- **Evaluate only** — run evaluation and produce a report without implementation when the user asks to only audit, review, evaluate, or report.
- **Improve** — jump to implementation when a previous evaluation already exists and the user asks to fix or improve the findings.
- **Quick mode** — auto-accept: evaluate and implement without discussion when the user asks for quick mode, auto-fix, or "just fix it".
