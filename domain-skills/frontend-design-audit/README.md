...
Then it implements the fixes when the active agent has local source access: accessibility attributes, semantic HTML, visual hierarchy improvements, design system extraction, interaction patterns, and more.
...
```text
Audit this UI for usability issues.
Review this page for accessibility and UX.
Run a quick design audit and fix safe issues.
Evaluate https://example.com for frontend usability.
```

The agent-compatible skill supports four modes based on the prompt:

- **Full audit:** evaluate, report, discuss, implement, verify
...
1. **Discover** — Reads front-end code or inspects a live page and identifies the interface type, tech stack, and user flows
...
4. **Implement** — Establishes a design foundation, applies fixes through that system, then runs a coherence pass to ensure everything holds together
5. **Verify** — Runs a post-implementation review to catch issues introduced by the fixes
